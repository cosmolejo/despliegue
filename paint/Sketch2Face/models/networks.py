...
def get_norm_layer(norm_type='instance'):
    if norm_type == 'batch':
        ...
        slrn_size = 11
        norm_layer = functools.partial(SLRN, size=slrn_size)
    else:
...
def define_G(input_nc, opt, gpu_ids=[]):    
    ...
    if opt.netG == 'global':    
        netG = GlobalGenerator(input_nc, opt.output_nc, opt.ngf, opt.n_downsample_global, opt.n_blocks_global, norm_layer)       
    elif opt.netG == 'dual': # dual generator
        netG = GlobalGenerator(input_nc, opt.output_nc, opt.ngf, opt.n_downsample_global, opt.n_blocks_global, norm_layer)         
        netG_sap = MaxPoolingGenerator_conv(input_nc=input_nc, 
                                    output_nc=opt.output_nc,
                                    ngf=opt.ngf,
                                    padding_type='reflect', 
                                    norm_layer=norm_layer, 
                                    n_downsampling=opt.n_downsample_global,
                                    n_blocks=opt.n_blocks_global,
                                    use_global_pooling=False, 
                                    sap_branches=opt.sap_branches,
                                    use_class_feat=opt.use_class_feat
                                    ) 
        
    elif opt.netG == 'local':        
        netG = LocalEnhancer(input_nc, opt.output_nc, opt.ngf, opt.n_downsample_global, opt.n_blocks_global, 
                                  opt.n_local_enhancers, opt.n_blocks_local, norm_layer)
    elif opt.netG == 'encoder':
        netG = Encoder(input_nc, opt.output_nc, opt.ngf, opt.n_downsample_global, norm_layer)
    else:
    ...
    if len(gpu_ids) > 0:
        ...
        netG.cuda(gpu_ids[0])
        if 'dual' in opt.netG:
            netG_sap.cuda(gpu_ids[0])

    if 'dual' in opt.netG:
        
        netG_sap.apply(weights_init)
        return netG, netG_sap
        
    netG.apply(weights_init)
    return netG
    
def define_E(input_nc, opt, gpu_ids=[]):    
    norm_layer = get_norm_layer(norm_type=opt.norm)     
    if opt.netG == 'encoder':
        netG = Encoder(input_nc, opt.output_nc, opt.ngf, opt.n_downsample_global, norm_layer)
    else:
...
############## SAP generator #######
class MaxPoolingGenerator_conv(nn.Module):
    '''
    Generator with SAP
    '''
    def __init__(self, input_nc, output_nc, kernel_size=3, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d, 
                 padding_type='reflect', use_global_pooling=False, sap_branches=[1,5,9,13], use_class_feat=False):
        assert(n_blocks >= 0)
        super(MaxPoolingGenerator_conv, self).__init__()        
        activation = nn.ReLU(True)        
        self.n_downsampling = n_downsampling
        self.pooling_nc = len(sap_branches)
        
        # MaxPoolingBlock do not use parameter from __init__
        self.max_pool = MaxPoolingBlock_conv(input_nc, 
                                    ngf=ngf,
                                    kernel_size=13, 
                                    padding_type='reflect', 
                                    norm_layer=norm_layer, 
                                    ...
                                    use_dropout=False,
                                    use_global_pooling=False,
                                    sap_branches=sap_branches,
                                    use_class_feat=use_class_feat)
        ...
        # len(model) = first*1 + down*n_downsampling + res*n_blocks + up*n_downsampling + final*1
        # len(model) = first*1 + down*3 + res*9 + up*3 + final*1
        self.num_layer = 1 + n_downsampling + n_blocks + n_downsampling + 1
        assert(len(model) == self.num_layer)
        for n in range(len(model)):
            if n <= n_downsampling: # first and down, do not load pretrained model
                setattr(self, 'model_'+str(n), nn.Sequential(*model[n]))
            else:
                setattr(self, 'model'+str(n), nn.Sequential(*model[n]))
            
    def forward(self, x):
        res = []
        pool_out, pool_attention = self.max_pool(x)
        res.append(pool_attention)
        res.append(pool_out)

        for n in range(self.n_downsampling  + 1):
            model = getattr(self, 'model_'+str(n))
            res.append(model(res[-1]))
        for n in range(self.n_downsampling + 1, self.num_layer):
            model = getattr(self, 'model'+str(n))
            res.append(model(res[-1]))
        # res == max_pool*1 + first*1 + down*3 + res*9 + up*3 + final*1
        # res[-1] == fake_image
        return res[-1], res
...
class MaxPoolingBlock_conv(nn.Module):
    ...
    def __init__(self, input_nc, 
        ...
        attention_block = [nn.Conv2d(atten_input_ch, ngf, kernel_size=kernel_size, padding=kernel_size//2),     # use large kernel size at the beginning
                            activation,
...
class Classifier(nn.Module):
    '''
    A simple fully-convolutional classifier for syn/dfm sketch classification.
    '''
    def __init__(self,  input_nc, label_num, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
        ...
        # last conv
        sequence += [[
                    nn.Conv2d(nf, label_num, kernel_size=kw, stride=1, padding=padw),
                    nn.AdaptiveAvgPool2d((1, 1))
                    ]]
...
class RaceClassifier(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32*32*192, 128)
