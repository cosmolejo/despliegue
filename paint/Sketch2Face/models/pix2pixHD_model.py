...
from paint.Sketch2Face.util.image_pool import ImagePool
...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss, use_gfm, use_edge_penalty, use_cross_entropy_race):
        flags = (True, use_gan_feat_loss, use_vgg_loss, True, True, use_gfm, use_edge_penalty,use_cross_entropy_race)
        def loss_filter(g_gan, g_gan_feat, g_vgg, d_real, d_fake, g_gfm, edge_penalty, cross_entropy_race):
            return [l for (l,f) in zip((g_gan, g_gan_feat, g_vgg,d_real, d_fake, g_gfm, edge_penalty, cross_entropy_race),flags) if f]
        return loss_filter
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_filter = self.init_loss_filter(not opt.no_ganFeat_loss, not opt.no_vgg_loss, opt.gfm, opt.use_edge_penalty, opt.race)
            ...
            if(opt.race):
                self.crossEntropyLossRace = torch.nn.CrossEntropyLoss()
        
            # Names so we can breakout loss
            self.loss_names = self.loss_filter('G_GAN','G_GAN_Feat','G_VGG','D_real', 'D_fake', 'G_GFM', 'edge_penalty','cross_entropy_race')
            ...
            if opt.niter_fix_global > 0 and 'dual'in opt.netG:   
                ...
                if 'dual' in opt.netG:
                    ...
                    if(opt.race):
                        ...
                        for key, value in params_dict.items(): 
                            layer_number = int(key.split('.')[0].split('model')[1])
                            if layer_number >= 15:
                                last_params_G += [value]
                        params = list(self.netR.parameters())+list(last_params_G)
    ...
    def encode_input(self, sketch, photo=None, deform_sketch=None, race = None):             
        ...
        if race is not None:
            race = Variable(race.cuda())
        if (self.opt.deform & self.opt.race):
            return sketch, photo, deform_sketch, race
        else:
            return sketch, photo
    ...
    def classify_race(self, photo):
        return self.netR.forward(photo)

    def forward(self, sketch, photo, sketch_deform=None, race = None, infer=False):
        # Encode Inputs
        sketch, real_image, sketch_deform, race = self.encode_input(sketch, photo, sketch_deform, race)
        ...
        #print(sketch.shape, real_image.shape)
        ...
        #Race Loss
        if(self.opt.race):
            pred_race = self.classify_race(sketch)
            loss_race = self.crossEntropyLossRace(pred_race, race)
        ...
        if self.opt.deform:
            ...
            if self.opt.use_edge_penalty:
                ...
                print(sketch.shape,pool_output_deform_.shape)
        ...
        _loss_race = loss_race if self.opt.race else 0
    ...
    def inference(self, sketch, photo=None, sketch_deform=None):
        ...
        sketch, photo = self.encode_input(Variable(sketch), photo)
    ...
    def update_learning_rate(self):
        ...
        if(self.opt.race):
            for param_group in self.optimizer_R.param_groups:
                param_group['lr'] = lr
        if self.opt.niter_fix_global > 0 and 'dual'in self.opt.netG:
            for param_group in self.optimizer_G_sap.param_groups:
                param_group['lr'] = lr
        else:
            if 'dual' in self.opt.netG:
